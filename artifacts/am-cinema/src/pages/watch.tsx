...
const PROVIDERS = [
  {
    // vidsrc-embed.ru — stable, supports sub_url injection
    id: "vidsrcembedru",
    label: "مصدر 1",
  ...
  {
    // vsembed.ru — vidsrc alternative, works in Gulf
    id: "vsembedru",
    label: "مصدر 3",
    supportsSubtitleUpload: false,
    buildUrl: (imdbId: string, type: string, season: string, episode: string) =>
  ...
  {
    // vaplayer.ru — best quality, supports external subtitle injection via sub_url
    id: "vidapiru",
    label: "مصدر 4",
    supportsSubUrl: true,
  ...
  {
    // Server-side torrent streaming via peerflix (works on Render)
    id: "webtorrent",
    label: "مصدر 5",
    isTorrent: true,
    supportsSubtitleUpload: true,
...
export default function Watch() {
  ...
  // Reset playerActivatedAt and watched state when episode/source changes
  useEffect(() => {
    playerActivatedAtRef.current = null;
  ...
  // AnyEmbed / player postMessage listener — track percent > 90 as "watched"
  useEffect(() => {
    if (isTorrentMode) return;

    function handlePlayerMessage(e: MessageEvent) {
      ...
      // Normalise percent from multiple player event formats
      let percent: number | null = null;

      // AnyEmbed format: { event: "progress", data: { percent: 95 } }
      if (payload["event"] === "progress") {
        const d = payload["data"] as Record<string, unknown> | undefined;
        if (typeof d?.["percent"] === "number") percent = d["percent"] as number;
      }

      // timeupdate format: { currentTime, duration }
      if (
        percent === null &&
        ...
        percent = ((payload["currentTime"] as number) / (payload["duration"] as number)) * 100;
      }

      // Direct percent format: { percent: 90 } or { progress: 90 }
      if (percent === null && typeof payload["percent"] === "number") {
        percent = payload["percent"] as number;
      }
      if (percent === null && typeof payload["progress"] === "number") {
        percent = payload["progress"] as number;
      }

      if (percent === null) return;

      setWatchedPercent(Math.round(percent));

      // Mark as watched once when percent exceeds 90
      if (percent >= 90 && !markedWatchedRef.current && media) {
        markedWatchedRef.current = true;
        saveWatchProgress(
          media.imdbId,
          9999, // sentinel: fully watched
          media.type === "series" ? season : undefined,
          media.type === "series" ? episode : undefined,
        );
        console.info("[PhoenixCinema] Marked as watched (>90%)");
      }
    }

    window.addEventListener("message", handlePlayerMessage);
    return () => window.removeEventListener("message", handlePlayerMessage);
  }, [isTorrentMode, media, season, episode]);
        ...
        {isTorrentMode ? (
          ...
          <>
            <iframe
              ...
              onError={() => {
                // Immediate fallback: iframe failed to load (network error / blocked)
                const currentIndex = iframeSources.findIndex((p) => p.id === providerId);
                const nextIndex = (currentIndex + 1) % iframeSources.length;
                if (nextIndex !== currentIndex) {
                  console.warn(`[PhoenixCinema] Source ${providerId} failed, switching to ${iframeSources[nextIndex].id}`);
                  setProviderId(iframeSources[nextIndex].id);
                  setClickGuardActive(true);
