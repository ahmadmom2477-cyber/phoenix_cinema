...
function WelcomeBanner() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("phoenix_user_name");
  ...
  return (
    <motion.div
      ...
      className="mx-4 md:mx-0 mb-4 flex items-center justify-between gap-3 px-4 py-3 rounded-2xl"
      style={{ background: "linear-gradient(135deg, rgba(220,38,38,0.12) 0%, rgba(220,38,38,0.04) 100%)", border: "1px solid rgba(220,38,38,0.2)" }}
    >
      <div className="flex items-center gap-3 min-w-0">
        <div className="w-9 h-9 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center shrink-0 text-base">
          👋
        </div>
        <div className="min-w-0">
          ...
          <p className="text-white/45 text-[11px] leading-tight">
            {t("استمتع بمشاهدة مفضلاتك", "Enjoy watching your favorites")}
          </p>
        </div>
      </div>
      <button
        onClick={() => setVisible(false)}
        className="shrink-0 w-7 h-7 flex items-center justify-center rounded-full text-white/30 hover:text-white/60 hover:bg-white/8 transition-all text-lg leading-none"
        aria-label="dismiss"
      >
        ×
      </button>
