...
function getSubToken(): string | null {
  try {
    const raw = localStorage.getItem("phoenix_subscription_v2");
    if (!raw) return null;
...
export function useFreeTrial() {
  ...
  const recordWatch = useCallback(async (
  ...
  ): Promise<{ allowed: boolean; remaining: number; reason?: string; message?: string }> => {
    ...
    try {
      ...
      if (data.consumed) {
        ...
        // For VPN blocks — don't drain plays, just block access
        const isVpn = data.reason === "vpn_blocked";
        const newStatus: AccessStatus = {
          ...(status ?? {
            isSubscribed: false,
            subscriptionExpiresAt: null,
            ...
            trialId: null,
            reason: isVpn ? "vpn_blocked" : "subscription_required",
          }),
          freePlaysRemaining: isVpn ? (status?.freePlaysRemaining ?? FREE_WATCH_LIMIT) : 0,
          canWatch: false,
          reason: isVpn ? "vpn_blocked" : "subscription_required",
        };
        cachedStatus = newStatus;
        setStatus(newStatus);
        window.dispatchEvent(new CustomEvent(EVENT, { detail: newStatus }));
        return { allowed: false, remaining: newStatus.freePlaysRemaining, reason: data.reason, message: data.message };
      }
    } catch {
