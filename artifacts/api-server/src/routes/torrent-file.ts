 ...
 * Serves the raw .torrent binary.
 * YTS is tried first because its .torrent files contain WebSeed URLs
 * (direct HTTP links to the video on YTS CDN), enabling WebTorrent to
 * start streaming immediately via HTTP even with zero BitTorrent peers.
 */
router.get("/torrent-file/:hash", async (req, res) => {
  ...
  for (const url of SOURCES) {
    try {
      ...
      if (new Uint8Array(buf)[0] !== 0x64) continue;
