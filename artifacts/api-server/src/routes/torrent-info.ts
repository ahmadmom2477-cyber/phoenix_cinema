...
router.get("/torrent-info/:imdbId", async (req, res) => {
  ...
  try {
    // ── Movies ────────────────────────────────────────────────────────────
    if (type !== "series") {
      // cat 207 = HD Movies, cat 200 = Movies
      ...
      // Deduplicate by hash, exclude 4K/2160p (too heavy for streaming)
      ...
      // Rank: quality first, then seeds
      const ranked = unique.sort((a, b) => {
        const qa = qualityFromName(a.name);
        const qb = qualityFromName(b.name);
        const ai = QUALITY_ORDER.indexOf(qa);
        const bi = QUALITY_ORDER.indexOf(qb);
    ...
    // ── Series ────────────────────────────────────────────────────────────
    // Search for specific episode first, then season pack
