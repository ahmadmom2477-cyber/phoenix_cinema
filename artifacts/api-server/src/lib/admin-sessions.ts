...
// ── Environment helpers ────────────────────────────────────────────────────────
// All env vars use PHOENIX_* prefix.

const DEFAULT_ADMIN_PASS = "abood2021";
const DEFAULT_SUB_SECRET = "phoenix-cinema-sub-secret-x7k2m9p3";

export function getAdminPass(): string {
  return process.env["PHOENIX_ADMIN_PASS"] ?? DEFAULT_ADMIN_PASS;
}

export function getSubSecret(): string {
  return process.env["PHOENIX_SUB_SECRET"] ?? DEFAULT_SUB_SECRET;
}

export function getPaymentUrl(): string | undefined {
  return process.env["PHOENIX_PAYMENT_URL"];
}
