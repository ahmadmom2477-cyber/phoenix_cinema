...
export function getAdminPass(): string {
  return process.env["PHOENIX_ADMIN_PASS"] ?? process.env["ENAWI_ADMIN_PASS"] ?? DEFAULT_ADMIN_PASS;
}

export function getSubSecret(): string {
  return process.env["PHOENIX_SUB_SECRET"] ?? process.env["ENAWI_SUB_SECRET"] ?? DEFAULT_SUB_SECRET;
}
