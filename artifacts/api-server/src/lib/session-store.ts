...
const IP_BINDING_FILE = join(DATA_DIR, "ip-bindings.json");

type IpBindingStore = Record<string, string>; // token → ip

function loadIpBindings(): IpBindingStore {
  try {
    if (!existsSync(IP_BINDING_FILE)) return {};
    return JSON.parse(readFileSync(IP_BINDING_FILE, "utf-8")) as IpBindingStore;
  } catch { return {}; }
}

function saveIpBindings(store: IpBindingStore): void {
  try {
    if (!existsSync(DATA_DIR)) mkdirSync(DATA_DIR, { recursive: true });
    writeFileSync(IP_BINDING_FILE, JSON.stringify(store, null, 2));
  } catch {}
}

export function bindTokenToIp(token: string, ip: string): void {
  if (!token || !ip || ip === "unknown") return;
  const store = loadIpBindings();
  store[token] = ip;
  saveIpBindings(store);
}

export function checkIpBinding(token: string, ip: string): boolean {
  if (!token || !ip || ip === "unknown") return true; // no IP = allow
  const store = loadIpBindings();
  const bound = store[token];
  if (!bound) return true; // token not bound yet = allow (backward compat)
  return bound === ip;
}
